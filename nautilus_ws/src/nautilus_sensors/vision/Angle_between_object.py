...
def switch_case_sub_angle(objects, mode, PREQUALIFICATION):
    results_angle = []
    ...
    if not PREQUALIFICATION:
        ...
        if slalom_side and slalom_middle:
            ...
            mean_distance = (right_obj["depth"] + left_obj["depth"]) / 2
            if angle_calc is not None and dist_center_gate is not None:
                results_angle.extend([float(ObjectID.SLALOM_SIDE_MID),float(dist_center_gate), float(mean_distance), float(angle_calc)])
        ...
        if gate_prequalif:
            ...
            if gate_L is not None and gate_R is not None:
                angle_calc = find_angle_plane_V2(gate_L["depth"],gate_R["depth"],2100, gate_L["box_cx"],gate_R["box_cx"], mode)
                dist_center_gate = find_dist_center_gate(cx, gate_L["box_cx"], gate_R["box_cx"])
